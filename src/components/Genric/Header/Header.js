      ...
      {data.map((item, index) => {
        return (
          <a
            key={index}
            href={item.url}
            className="underline-offset-2 hover:underline text-yellow-700 shadow-[0px_0px_10px_#ccc] p-1 rounded-[10px] border"
          >
            {item.name}
          </a>
        );
      ...
      <div className="container h-[87px]">
        <div className="flex justify-between h-[87px] px-2 md:px-0">
          ...
          <div className="hidden lg:flex justify-around items-center gap-20">
            <a href="/" className=" lg:flex text-[15px] font-semibold text-black">
        ...
        <div
          className={`lg:hidden shadow-[0px_10px_10px_#ccc] h-0 flex-col w-full absolute top-[100px] left-0 bg-white overflow-hidden block transition-max-height duration-500 ease-in-out ${
            isMobileMenuOpen ? "flex h-auto max-h-screen" : "max-h-0 overflow-hidden"
          }`}
        >
